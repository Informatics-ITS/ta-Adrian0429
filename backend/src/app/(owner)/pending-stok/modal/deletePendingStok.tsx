...
export function DeletePendingStokModal({
  children,
  restok_id,
}: {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return await api.delete(`/api/produk/pending/${id}`);
    },
    onSuccess: () => {
      toast.success(`Berhasil menghapus Pending Stok!`);
      router.push("/pending-stok");
    },
    onError: (err) => {
      ...
      >
        ...
        <Modal.Body className="text-center px-10 mt-5">
          ...
          <p className="text-S1 text-[#6B7280] mt-[10px]">
            Apakah anda yakin untuk menghapus ajuan stok dengan id {restok_id}?
          </p>
        </Modal.Body>
        ...
        <Modal.Footer className="mt-5 mb-8">
          <div className="flex justify-center gap-4">
            ...
            <Button
              ...
              onClick={() => {
                onClose();
                deleteMutation.mutate(restok_id);
              }}
            >
              Ya
            </Button>
